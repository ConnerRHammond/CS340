...
public abstract class DynamoBaseDAO {
    ...
    public ItemCollection<QueryOutcome> tableQueryExecuter(HashMap<String, String> nameMap, HashMap<String, Object> valueMap, Table table, String Expression, int Max, ArrayList key) {
        QuerySpec querySpec;
        if (Max != 0) {
            if (key != null) {
                ...
                querySpec = new QuerySpec().withKeyConditionExpression(Expression).withNameMap(nameMap)
                        .withValueMap(valueMap).withScanIndexForward(true).withMaxResultSize(Max);
            }
        } else {
            querySpec = new QuerySpec().withKeyConditionExpression(Expression).withNameMap(nameMap)
                    .withValueMap(valueMap).withScanIndexForward(true);
