...
public class DynamoFollowDAO extends DynamoBaseDAO implements FollowsDAO {
    ...
    public boolean isFollower(String follower, String followee) {
        ...
        ItemCollection<QueryOutcome> items = tableQueryExecuter(nameMap,valueMap,table,"#f = :g and #h = :t",0,null,true);
    ...
    public CountResponse getFollowerCount(String Alias){
        ...
        return new CountResponse(items.getAccumulatedItemCount());
    }
    @Override
...
//         return new CountResponse(numberOfMatches(items));
    ...
    @Override
    public Pair<List<String>, Boolean> getFollowers(String Alias, int max, ArrayList key) {
        HashMap<String, String> nameMap = new HashMap<String, String>();
        nameMap.put("#f", "followee_handle");
        HashMap<String, Object> valueMap = new HashMap<String,Object>();
        valueMap.put(":g",Alias);
        ItemCollection<QueryOutcome> items = indexQueryExecuter(nameMap,valueMap,table.getIndex("followee_handle"),"#f = :g",max,key);
        Iterator<Item> iterator = null;
        Item item = null;
    ...
    public Pair<List<String>, Boolean> getFollowing(String Alias,int max, ArrayList key) {
        ...
        ItemCollection<QueryOutcome> items = tableQueryExecuter(nameMap,valueMap,table,"#f = :g",max,key,true);
        Iterator<Item> iterator = null;
        Item item = null;
        ArrayList<String> users = new ArrayList<>();
    ...
    public QuerySpec IndexQuery(HashMap<String, String> nameMap, HashMap<String, Object> valueMap, int Max, String Expression, ArrayList key) {
        QuerySpec querySpec = new QuerySpec().withKeyConditionExpression("#f = :g").withNameMap(nameMap)
                .withValueMap(valueMap).withScanIndexForward(true).withMaxResultSize(Max).withExclusiveStartKey("followee_handle",key.get(0),"follower_handle",key.get(1));
        return querySpec;
    }
}
