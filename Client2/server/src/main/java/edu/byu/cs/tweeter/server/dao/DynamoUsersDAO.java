...
public class DynamoUsersDAO extends DynamoBaseDAO implements UsersDAO {
    ...
    @Override
    public int getFollowers(String alias) {
    ...
    public void incrementFollowers(String alias) {
        ...
        try{
            iterator = items.iterator();
            item = iterator.next();
            int i = item.getInt("followers")+1;
            UpdateItemSpec updateItemSpec = new UpdateItemSpec().withPrimaryKey("alias", alias).withUpdateExpression("set followers = :val")
                    .withValueMap(new ValueMap().withInt(":val",i));
    ...
    public String createUser(String alias,String firstName,String lastName, String url,String password) {
        try {
            ...
            create(new Item().withPrimaryKey("alias", alias)
                    ...
                    .withString("imageUrl", url).withString("password",password).withInt("followers",0), table);
            return "Success";
        }catch (Exception e){
            return e.toString();
        }
